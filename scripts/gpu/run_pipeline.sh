...
run_full() {
    echo "Running Full pipeline (Splat Aerial + Quality-Gated Ground GLB)..."
    notify_slack "Starting full pipeline with $(ls -1 "$INPUT_DIR"/*.jpg "$INPUT_DIR"/*.jpeg "$INPUT_DIR"/*.png 2>/dev/null | wc -l | tr -d ' ') images"

    # === Phase 1: ODM Orthomosaic + DSM/DTM (shared foundation) ===
    local ODM_OUTPUT="$OUTPUT_DIR/odm"
    local ODM_TAR="/mnt/splatwalk/odm-docker.tar.gz"
    local ODM_ORTHO_TIF="$ODM_OUTPUT/odm_orthophoto/odm_orthophoto.tif"
    if [ ! -f "$ODM_TAR" ]; then
    ...
    notify_slack "Phase 1: ODM orthomosaic + DSM/DTM..."
    docker load -i "$ODM_TAR" 2>/dev/null
    mkdir -p "$ODM_OUTPUT"
    docker run --rm \
        -v "$INPUT_DIR:/datasets/project/images" \
        ...
        2>&1 | tail -50 \
        || { notify_slack "FATAL: ODM failed" "error"; return 1; }
    notify_slack "Phase 1 complete: ODM orthomosaic + DSM/DTM"
...
" 2>&1)" || true
    ...
    # === Phase 3: Splat Stage 1 — MASt3R geometry init ===
    local scene_dir="$OUTPUT_DIR/scene"
    mkdir -p "$scene_dir/images"
...
PREPROCESS_SCRIPT
    ...
    notify_slack "Phase 3: Geometry init (MASt3R, $n_views views)..."
    python init_geo.py \
        --source_path "$scene_dir" \
        --model_path "$OUTPUT_DIR/instantsplat" \
        --n_views "$n_views" \
        ...
        --conf_aware_ranking \
        2>&1 || {
            notify_slack "Failed at Phase 3: init_geo.py" "error"
            return 1
        }
    notify_slack "Phase 3 complete: point cloud generated"

    # === Phase 4: Splat Stage 2 — InstantSplat training (10K iterations) ===
    notify_slack "Phase 4: Training aerial splat ($train_iters iterations)..."
    python train.py \
        --source_path "$scene_dir" \
        --model_path "$OUTPUT_DIR/instantsplat" \
        --iterations "$train_iters" \
        ...
        --optim_pose \
        || {
            notify_slack "Failed at Phase 4: train.py" "error"
            return 1
        }
    notify_slack "Phase 4 complete: aerial splat trained"

    # === Phase 5: Zoom Descent (5 altitude levels, top-down only) ===
    notify_slack "Phase 5: Top-down zoom descent (5 altitude levels)..."
    local descent_args=(
        --model_path "$OUTPUT_DIR/instantsplat"
        --scene_path "$scene_dir"
        --output_dir "$OUTPUT_DIR/descent"
    ...
    python /mnt/splatwalk/scripts/render_zoom_descent.py "${descent_args[@]}" \
        || {
            notify_slack "Phase 5 failed (zoom descent) — continuing with base splat" "info"
        }

    local final_model_path="$OUTPUT_DIR/descent/final"
    if [ ! -d "$final_model_path" ]; then
        final_model_path="$OUTPUT_DIR/instantsplat"
        echo "  Using base splat (no descent output)"
    fi
    notify_slack "Phase 5 complete: zoom descent finished"

    # === Phase 6: Ground GLB (game-level scene with PBR + vegetation) ===
    notify_slack "Phase 6: Building ground-level scene GLB..."

    # Generate aerial GLB for scene_transform.json (needed by build_game_scene)
    python /mnt/splatwalk/scripts/generate_aerial_glb.py \
        --orthophoto "$ODM_ORTHO_TIF" \
        --dsm "$ODM_OUTPUT/odm_dem/dsm.tif" \
        --output "$OUTPUT_DIR/aerial.glb" \
        ...
        --upscale \
        --scene_transform_out "$OUTPUT_DIR/scene_transform.json" \
        || notify_slack "Aerial GLB generation failed (non-fatal for full pipeline)" "info"
    ...
    python /mnt/splatwalk/scripts/build_game_scene.py $BUILD_ARGS \
        || {
            notify_slack "Phase 6 (ground GLB) failed (non-fatal)" "info"
            echo "WARNING: Ground GLB failed, aerial-only mode"
        }

    # === Phase 7: Quality Loop (Gemini scoring + iteration) ===
    if [ -f "$OUTPUT_DIR/scene.glb" ] && [ -n "${GEMINI_API_KEY:-}" ]; then
        notify_slack "Phase 7: Gemini quality loop..."
        python /mnt/splatwalk/scripts/gemini_score_scene.py \
            ...
            --slack_webhook_url "${SLACK_WEBHOOK_URL:-}" \
            --job_id "${JOB_ID:-}" \
            || notify_slack "Phase 7 (quality loop) failed (non-fatal)" "info"
        notify_slack "Phase 7 complete"
    else
        echo "Skipping quality loop (no scene.glb or no Gemini API key)"
    fi

    # === Phase 8: Compress Splat ===
    notify_slack "Phase 8: Compressing splat..."
    local ply_path=""
    for search_dir in "$final_model_path" "$OUTPUT_DIR/instantsplat"; do
    ...
    if [ -n "$ply_path" ] && [ -f "$ply_path" ]; then
        python /mnt/splatwalk/scripts/compress_splat.py \
            ...
            --scene_scale 50.0 \
            || {
                notify_slack "Phase 8 (compress splat) failed" "error"
                return 1
            }
        notify_slack "Phase 8 complete: splat compressed"
    else
        notify_slack "Phase 8: No PLY found, skipping compression" "error"
        return 1
    fi
...
"
    ...
    "full")
        if run_full; then
            notify_slack "Uploading splat + GLB assets..."
            ...
            notify_slack "Pipeline complete! Splat + GLB assets uploaded" "success"
        else
            notify_slack "Full pipeline failed" "error"
            exit 1
        fi
        ;;
    "mesh")
