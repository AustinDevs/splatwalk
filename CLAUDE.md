...
SplatWalk — a static demo site showcasing top-down aerial fly-over of land parcels using Gaussian Splatting. A progressive zoom pipeline generates high-detail overhead splats from drone photos, viewed as an interactive fly-over in the browser. Assets are generated by an ephemeral DigitalOcean H100 GPU pipeline and hosted on DO Spaces CDN.
...
3. **Stage 3**: Top-down progressive zoom descent (`render_zoom_descent.py`)
   ...
   - FLUX ControlNet enhances renders at each level (depth as structure, nearest aerial crop as IP-Adapter reference)
...
- FLUX generating a zoomed-in crop of terrain from above is much easier than inventing a ground-level perspective
...
- **Never retrain with FLUX-generated images at same altitude** — add at lower altitudes only
